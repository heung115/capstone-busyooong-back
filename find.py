...
def shortestBusRoute(pathId, userStartId, userEndId, cityCode):
    ...
    for routeId in pathId:
        startOrd, endOrd = 0, 0
        params["routeId"] = routeId
        totalCount = requests.get(url, params=params).json()['response']['body']['totalCount']
...
def findBus(userLati, userLong, userDestination):
    ...
    res1 = []
    datas = shortestBusRoute(crossPathIds, userStart[1], userEnd[1], userStart[-1])

    res2 = []
    for data in datas:
        url = 'http://apis.data.go.kr/1613000/BusRouteInfoInqireService/getRouteInfoIem'
        params ={'serviceKey' : getEnv("DATA_GO_KEY"), '_type' : 'json', 'cityCode' : userEnd[-1], 'routeId' : data }

        routeId = requests.get(url, params=params).json()['response']['body']['items']['item']['routeno']
        
        url = 'http://apis.data.go.kr/1613000/ArvlInfoInqireService/getSttnAcctoArvlPrearngeInfoList'
        params ={'serviceKey' : getEnv("DATA_GO_KEY"), 'pageNo' : '1', 'numOfRows' : '10', '_type' : 'json', 'cityCode' : userEnd[-1], 'nodeId' : data }

        response = requests.get(url, params=params).json()

        routePredictTime = -1
        if response['response']['body']['totalCount'] != 0:
            routePredictTime = int(response["response"]["body"]["items"]["item"][0]["arrtime"])
        
        res2.append({
            'routeId': routeId,
            'routePredictTime': routePredictTime
        })
